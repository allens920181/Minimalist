...
export function AuthGate({ children }: AuthGateProps) {
  ...
  useEffect(() => {
    // Capture result after redirect from Google
    ...
    return (
      <div className="flex h-screen items-center justify-center bg-gradient-to-br from-slate-50 via-white to-indigo-50/30">
        <div className="bg-white rounded-3xl shadow-[0_20px_60px_rgba(0,0,0,0.08)] border border-slate-100 p-10 flex flex-col items-center gap-6 max-w-sm w-full mx-4">
          ...
          {error && (
            <div className="w-full flex items-start gap-2 bg-red-50 border border-red-100 rounded-xl p-3 text-sm text-red-700">
          ...
          <button
            ...
            disabled={signingIn}
            className="w-full flex items-center justify-center gap-3 px-6 py-3 bg-white border border-slate-200 rounded-xl text-sm font-medium text-slate-700 hover:bg-slate-50 hover:border-slate-300 shadow-sm transition-all duration-200 active:scale-[0.98] disabled:opacity-60 disabled:cursor-not-allowed"
          >
            {signingIn ? (
          ...
          <p className="text-xs text-slate-400 text-center">
            Your data is privately synced to your Google account.
          </p>
        </div>
      </div>
    );
